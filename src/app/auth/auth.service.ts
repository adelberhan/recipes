...
export class AuthService {
  ...
  private SignInEndPoint = `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${this.apiKey}`;
  ////////////////
  constructor(private http: HttpClient, private router: Router) {}

  private handelUserAutonation(
    email: string,
    userId: string,
    token: string,
    expiresIn: number
  ...
  signIn(email: StringConstructor, password: string) {
    return this.http
  ...
  autoLogin() {
    ...
    if (loadedUser.token) {
      ...
      const expLoadedUserSession =
        new Date(userData.tokenExpirationDate).getTime() - new Date().getTime();
      this.autoLogout(expLoadedUserSession);
    }
  }
